...
'table
('valid? 'from-columns 'from-rows 'from-records 'column 'cast 'select 'rename
 'with-column 'aggregate 'inner-join 'left-join-with 'rows 'records)
import
...
[] ({"r" ["e"] "v" [1]} ["r"] [["t" "zz" ("reached" fail)]] aggregate) @attempt 'err at 'kind at
io.pp

# A quotation of the wrong shape is 'contract, reported by the applying word.
[] ({"r" ["e"] "v" [1]} ["r"] [["t" "v" (dup)]] aggregate) @attempt 'err at dup 'kind at io.pp 'word
at
io.pp
...
broken type io.pp
broken valid? io.pp
broken wrap (rows) @attempt 'err at 'kind at io.pp
broken wrap (records) @attempt 'err at 'kind at io.pp
