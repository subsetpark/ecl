...
(
 ...
 (resource operation request -- value :
  "Begin an operation, claim its result, and close the exchange. Use only when no additional
   ...
   error.")
 (begin dup wrap (result) @attempt swap close result.or-raise first) 'call def
) 'port @defm
