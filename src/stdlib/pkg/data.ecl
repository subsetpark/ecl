...
(
 ...
 (|key holder| key holder key at pair) 'entry-of defp

 ### def sorted-entries
 (holder -- pairs : "Return a dict's entries in ascending key order.")
 (|holder| holder dict.keys sort holder (entry-of) partial each) 'sorted-entries def
) 'pkg.data @defm
