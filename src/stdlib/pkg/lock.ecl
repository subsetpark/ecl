...
(
 ...
 (|selection|
  ...
  error.with-message assert
  selection) 'selection-checked defp
 ...
 (|minimums|
  ...
  error.with-message assert
  minimums) 'minimums-checked defp

 ### defp known?
 (pair packages -- bool : "Test whether a required package has a locked selection.")
 (|pair packages| packages pair 1 at 'package at dict.has?) 'known? defp
 ...
 (|entry packages|
  ...
  entry 1 at 'version at pkg.version.less? not) 'satisfied? defp
 ...
 (pkg.data.read-one pkg.lock.validate) 'read def

 ### def vendor
 (lock -- lock : "Return a validated lock selecting the fixed project-local vendor store.")
 (pkg.lock.validate 'store 'vendor put pkg.lock.validate) 'vendor def
 ...
 ((first str) (1 at render-requirement) bi
  2 pack "{} {}" str.format) 'render-selection defp
 ...
 (|pair|
  ...
  3 pack
  "{} {{'package {} 'version {}}}" str.format) 'render-minimum defp
 ...
 (|line state| state 'lines state 'lines at line append put) 'append-tree-line defp
 ...
 (|state entry|
  ...
  infra
  "{} -> {} {}" str.format
  state append-tree-line) 'tree-edge defp
 ...
 ((match?) partial any?) 'path-has? defp
