### module pkg
# Validate, parse, and format `ecl.pkg` and `ecl.lock` data. Operations accept
# values or text and perform no file, network, or evaluation effects.
(# Decimal digits.
 "0123456789"
 'digit-chars setp
 ...
 ### def related?
 (|left right| left right owns-prefix? right left owns-prefix? or)
 ...
 ### def satisfied?
 (|pair packages| packages pair first at 'version at pair 1 at version< not)
 ...
 ### def render-minimums
 (|minimums| "{" minimums sorted-entries (render-minimum) each " " join "}" 3 pack raze)
