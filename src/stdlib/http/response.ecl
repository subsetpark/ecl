...
(
 ...
 (dup type 'int match? (dup 100 >= swap 599 <= and) (pop 0) if) 'status? defp

 ### defp header-values?
 (value -- bool : "Return 1 for a string or a list of strings.")
 (dup str.str? (pop 1) (dup type 'list match? ((str.str?) all?) (pop 0) if) if) 'header-values? defp

 ### defp header-pair?
 (pair -- bool : "Return 1 for a [name values] entry with a string name.")
 (dup first str.str? (1 at header-values?) (pop 0) if) 'header-pair? defp
 ...
 (dup valid? "expected a response dict {'status 'headers 'body}" type-error assert) 'checked defp

 ### defp checked-status
 (status -- status : "Return a status int in 100...599 or raise 'type.")
 (dup status? "http.response expects an int status in 100...599" type-error assert) 'checked-status
 defp

 ### defp checked-name
 (name -- name : "Return a string header name or raise 'type.")
 (dup str.str? "http.response expects a string header name" type-error assert) 'checked-name defp

 ### def new
 (status -- response :
  "Return {'status status 'headers {} 'body \"\"}. A status outside 100...599 is 'type.")
 (checked-status 'status swap 'headers {} 'body "" 6 pack dict.from-flat) 'new def
 ...
 (404 "not found" text) 'not-found def
 ...
 (response name -- values :
  "Return every value of a header as a list, matching the name regardless of letter case and in dict
   ...
   name is 'type.")
 (checked-name swap checked 'headers at dict.pairs swap (values-of) partial each raze) 'header def
 ...
 (checked 'status at 100 div) 'class def
 ...
 (checked-status swap checked 'status rolldown put) 'with-status def
 ...
 (dup headers? "http.response.with-headers expects a dict of headers" type-error assert
  swap checked dup 'headers at rolldown dict.merge 'headers swap put) 'with-headers def
) 'http.response @defm
