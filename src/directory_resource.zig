...
const Stream = @import("filesystem_stream.zig").Stream;
...
fn Handle(comptime Object: type) type {
    return struct {
        const Cell = @This();
        const Active = struct { object: Object, access: *external.FilesystemAccess, leases: usize = 0, reservation: ?*fs.Reservation = null };
        issuer: *Identity,
        ...
        writes: if (Object == *Stream) WriterLane else void,
        ...
        pub fn readyLocked(self: *Cell, key: u64) bool {
            if (Object == *Stream) {
                if (key == 2) return self.phase != .open or self.writes.hasCapacity(16);
                if (key >= 3) {
                    if (self.phase != .open and self.phase != .sealing) return true;
                    return self.writes.writerReady(key);
                }
            }
...
const StreamCell = Handle(*Stream);
...
pub fn adoptEnumeration(access: *external.FilesystemAccess, scope: *scheduler.TaskScope, dir: std.Io.Dir, parent: Value, ticket: ?*fs.Reservation) error{ OutOfMemory, ScopeClosing }!Value {
    ...
    return adoptReservedHandle(Enumeration, access, scope, .{ .dir = dir, .iterator = dir.iterate() }, group, ticket);
}
...
pub fn next(item: Value) Next {
    ...
    var admission: ?fs.OperationSlot = if (cell.phase.open.reservation) |ticket| ticket.claim() orelse return .{ .failed = .limit } else null;
    defer if (admission) |*slot| slot.release();
...
fn adoptHandle(comptime Object: type, access: *external.FilesystemAccess, scope: *scheduler.TaskScope, dir: Object, dependent: ?*scheduler.ExternalGroup) error{ OutOfMemory, ScopeClosing }!Value {
    return adoptReservedHandle(Object, access, scope, dir, dependent, null);
}
// Consumes both the object and its optional reservation on either outcome.
fn adoptReservedHandle(comptime Object: type, access: *external.FilesystemAccess, scope: *scheduler.TaskScope, dir: Object, dependent: ?*scheduler.ExternalGroup, reservation: ?*fs.Reservation) error{ OutOfMemory, ScopeClosing }!Value {
    ...
    const cell = issuer.allocator().create(ResourceCell) catch |err| {
        if (Object == *Stage or Object == *Stream) dir.retire() else dir.close(io);
        if (reservation) |ticket| ticket.release();
        return err;
    };
    issuer.retain();
    cell.* = .{ .issuer = issuer, .allocator = issuer.allocator(), .io = io, .phase = .{ .open = .{ .object = dir, .access = access, .reservation = reservation } }, .writes = if (Object == *Stream) .init(&cell.mutex) else {} };
...
pub fn adoptStage(access: *external.FilesystemAccess, scope: *scheduler.TaskScope, stage: *Stage, parent: Value, ticket: ?*fs.Reservation) error{ OutOfMemory, ScopeClosing }!Value {
    const group = dependentGroup(parent, scope) catch |err| {
        stage.retire();
        return err;
    };
    defer if (group) |g| g.release();
    if (ticket) |reservation| reservation.retain();
    return adoptReservedHandle(*Stage, access, scope, stage, group, ticket);
}
...
pub fn adoptChild(access: *external.FilesystemAccess, scope: *scheduler.TaskScope, dir: std.Io.Dir, parent: Value, ticket: ?*fs.Reservation) error{ OutOfMemory, ScopeClosing }!Value {
    const group = dependentGroup(parent, scope) catch |err| {
        dir.close(fs.hostIo(access));
        return err;
    };
    defer if (group) |g| g.release();
    if (ticket) |reservation| reservation.retain();
    return adoptReservedHandle(std.Io.Dir, access, scope, dir, group, ticket);
}

/// Consumes a stream on either outcome. Its root lease joins directory close;
/// permanent staging dependency also survives a task transfer.
pub fn adoptStream(access: *external.FilesystemAccess, scope: *scheduler.TaskScope, stream: *Stream, parent: Value) error{ OutOfMemory, ScopeClosing }!Value {
    const group = dependentGroup(parent, scope) catch |err| {
        stream.retire();
        return err;
    };
    defer if (group) |g| g.release();
    return adoptHandle(*Stream, access, scope, stream, group);
}

pub fn isStream(item: Value) bool {
    return resource.Resource.project(StreamCell, item) != null;
}

const WriteResult = union(enum) { written, pending, failed: fs.Reason };
const WritePreparation = union(enum) { ready, pending, failed: fs.Reason };
const WriterLane = @import("port_controller.zig").Lane(StreamCell, .writer, .{
    .retain = retainWriter,
    .release = releaseWriter,
    .write = writeStreamLocked,
    .notify = notifyStreamLocked,
    .source = writerSource,
});
fn retainWriter(cell: *StreamCell) void {
    cell.phase.open.leases += 1;
    cell.retainReadiness();
}
fn releaseWriter(cell: *StreamCell) void {
    cell.releaseLease();
    cell.releaseReadiness();
}
fn notifyStreamLocked(cell: *StreamCell) void {
    cell.waits.notifyLocked(cell);
}
fn writerSource(cell: *StreamCell, key: u64) external.ReadinessSource {
    return external.readinessSource(StreamCell, cell, key);
}
fn writeStreamLocked(cell: *StreamCell, active: bool, bytes: []const u8) WritePreparation {
    const stream = switch (cell.phase) {
        .open, .sealing => |current| current.object,
        .closing, .retiring, .closed => return .{ .failed = .io },
    };
    if (!active) return .pending;
    if (stream.prepareAppend(bytes.len)) |reason| return .{ .failed = reason };
    return .ready;
}

pub const WriteClaim = opaque {
    const State = struct { cell: *StreamCell, stream: *Stream, permit: *WriterLane.Writer, complete: bool = false };
    fn state(self: *WriteClaim) *State {
        return @ptrCast(@alignCast(self));
    }
    pub fn remaining(self: *WriteClaim) u64 {
        const owned = self.state();
        std.Io.Threaded.mutexLock(&owned.cell.mutex);
        defer std.Io.Threaded.mutexUnlock(&owned.cell.mutex);
        return owned.stream.remaining();
    }
    pub fn ready(self: *WriteClaim) bool {
        const owned = self.state();
        std.Io.Threaded.mutexLock(&owned.cell.mutex);
        defer std.Io.Threaded.mutexUnlock(&owned.cell.mutex);
        return (owned.cell.phase != .open and owned.cell.phase != .sealing) or owned.permit.active();
    }
    pub fn source(self: *WriteClaim) external.ReadinessSource {
        return self.state().permit.source();
    }
    pub fn fail(self: *WriteClaim, reason: fs.Reason) void {
        const owned = self.state();
        std.Io.Threaded.mutexLock(&owned.cell.mutex);
        owned.stream.fail(reason);
        std.Io.Threaded.mutexUnlock(&owned.cell.mutex);
    }
    pub fn append(self: *WriteClaim, bytes: []const u8) WriteResult {
        const owned = self.state();
        switch (owned.permit.write(bytes)) {
            .pending => return .pending,
            .failed => |reason| return .{ .failed = reason },
            .ready => {},
        }
        const failure_reason = owned.stream.write(bytes);
        std.Io.Threaded.mutexLock(&owned.cell.mutex);
        defer std.Io.Threaded.mutexUnlock(&owned.cell.mutex);
        if (owned.cell.phase != .open and owned.cell.phase != .sealing) owned.stream.fail(.io);
        if (owned.stream.finishAppend(bytes.len, failure_reason)) |reason| return .{ .failed = reason };
        owned.complete = true;
        return .written;
    }
    /// Consumes the claim. Interrupted construction poisons future publication.
    pub fn deinit(self: *WriteClaim) void {
        const owned = self.state();
        const permit = owned.permit;
        const complete = owned.complete;
        if (!complete) self.fail(.io);
        owned.cell.resourceAllocator().destroy(owned);
        if (complete) permit.finish() else permit.cancel();
    }
};

pub const WriteAdmission = union(enum) { claimed: *WriteClaim, pending: external.ReadinessSource, failed: fs.Reason, closed };
pub fn acquireWrite(item: Value) error{OutOfMemory}!WriteAdmission {
    const cell = resource.Resource.project(StreamCell, item) orelse return .closed;
    var candidate: ?*WriteClaim.State = try cell.resourceAllocator().create(WriteClaim.State);
    defer if (candidate) |owned| cell.resourceAllocator().destroy(owned);
    var prepared: ?*WriterLane.Prepared = try cell.writes.prepare(cell.resourceAllocator());
    defer if (prepared) |owned| owned.discard();
    std.Io.Threaded.mutexLock(&cell.mutex);
    defer std.Io.Threaded.mutexUnlock(&cell.mutex);
    if (cell.phase != .open) return .closed;
    const stream = cell.phase.open.object;
    if (stream.failure()) |reason| return .{ .failed = reason };
    const permit = prepared.?.admitWriter(cell, 16) orelse return .{ .pending = external.readinessSource(StreamCell, cell, 2) };
    prepared = null;
    const owned = candidate.?;
    owned.* = .{ .cell = cell, .stream = stream, .permit = permit };
    candidate = null;
    return .{ .claimed = @ptrCast(owned) };
}
...
pub fn commit(item: Value) Commit {
    ...
    return commitCell(StageCell, cell);
}
pub fn commitFile(item: Value) Commit {
    const cell = resource.Resource.project(StreamCell, item) orelse return .closed;
    return commitCell(StreamCell, cell);
}
fn commitCell(comptime Cell: type, cell: *Cell) Commit {
    std.Io.Threaded.mutexLock(&cell.mutex);
    if (cell.phase == .open) {
    ...
    if (cell.phase.sealing.leases != 0 or cell.children == .open) {
        ...
        return .{ .pending = external.readinessSource(Cell, cell, 1) };
    }
    var admission: ?fs.OperationSlot = if (cell.phase.sealing.reservation) |ticket| ticket.claim() orelse {
        std.Io.Threaded.mutexUnlock(&cell.mutex);
        return .{ .failed = .limit };
    } else null;
    defer if (admission) |*slot| slot.release();
...
const LeaseState = struct { origin: Origin, io: std.Io, dir: std.Io.Dir, ticket: ?*fs.Reservation };
pub const Lease = opaque {
    ...
    pub fn reservation(self: *Lease) ?*fs.Reservation {
...
fn acquireCell(comptime Cell: type, cell: *Cell, origin: Origin) error{ OutOfMemory, Closed, Io }!*Lease {
    ...
    cell.retainReadiness();
    cell.phase.open.leases += 1;
    owned.* = .{ .origin = origin, .io = cell.io, .dir = dir, .ticket = cell.phase.open.reservation };
