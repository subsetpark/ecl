...
const OperationBackend = @import("native_port.zig").OperationBackend;
...
/// Native exchange lifetime owns cancellation settlement, terminal result
/// publication, and scope lifetime independently of backend callbacks.
pub const Exchange = struct {
    ...
    const Preparation = struct {
        allocator: std.mem.Allocator,
        node: *Lane.Prepared,
        phase: union(enum) { ready: struct { adapter: OperationBackend, result: *results.Result, mode: Mode }, admitted },
    };
    ...
    pub fn prepare(adapter: OperationBackend, result: *results.Result, lane: *Lane, mode: Mode) error{OutOfMemory}!*Prepared {
        ...
        owned.phase.ready.adapter.cell.retainReadiness();
        return @ptrCast(owned);
    }
    fn initialize(self: *Operation, ticket: *Lane.Ticket, adapter: OperationBackend, result: *results.Result, mode: Mode) void {
    ...
    pub fn publish(self: *Operation, identity: u64, scope: *scheduler.TaskScope) error{ OutOfMemory, ScopeClosing }!Value {
        ...
        const resource_mutex = &self.adapter.cell.mutex;
        lock(resource_mutex);
        lock(&self.mutex);
    ...
    fn runnable(self: *Operation) bool {
        return !self.adapter.cell.closed.load(.acquire);
    }
    ...
    pub fn childGroup(self: *Operation) error{ OutOfMemory, Closed }!*scheduler.ExternalGroup {
        ...
        const candidate = try scheduler.ExternalGroup.create(self.adapter.cell.scheduler, Operation, self);
        lock(&self.mutex);
