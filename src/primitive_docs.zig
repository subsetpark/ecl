...
const entries = [_]Metadata{
    ...
    .{ .name = "slurp", .effect = "path -- string", .text = "Read one whole UTF-8 file as a string." },
    .{ .name = "spit", .effect = "string path --", .text = "Write a string to one file, truncating and replacing it." },
    .{ .name = "getenv", .effect = "name -- string", .text = "Return an environment variable's value from the session snapshot." },
    .{ .name = "stdin", .effect = "-- string", .text = "Read the whole standard input stream once as a string." },
    ...
    .{ .name = "while", .effect = "cond body -- ...", .text = "Run a body quotation while a condition quotation returns the boolean 1." },
    .{ .name = "times", .effect = "n quotation -- ...", .text = "Run a quotation the requested nonnegative number of times." },
    .{ .name = "cond", .effect = "clauses -- ...", .text = "Run the first action whose test quotation returns the boolean 1, or the final else quotation." },
    ...
    .{ .name = "@module", .effect = "body name --", .text = "Evaluate a module body in a fresh unit and register its definitions under a name." },
    ...
    .{ .name = "@spawn", .effect = "quotation -- task", .text = "Run a quotation concurrently in a fresh child unit." },
    ...
    .{ .name = "@each", .effect = "sequence quotation -- results", .text = "Apply a quotation concurrently in one fresh unit per element and return one result per element in input order." },
