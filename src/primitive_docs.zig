...
const entries = [_]Metadata{
    ...
    .{ .name = "each", .effect = "collection quotation -- collection", .text = "Apply a one-input, one-output quotation independently to each list element or dictionary value, preserving dictionary keys." },
    .{ .name = "zip-with", .effect = "left right quotation -- list", .text = "Apply a two-input, one-output quotation across two conforming lists." },
    .{ .name = "for", .effect = "collection quotation --", .text = "Apply a one-input, zero-output quotation to each list element or dictionary value in order." },
    ...
    .{ .name = "@module", .effect = "values body -- module", .text = "Evaluate a module body with an explicit initial stack in a fresh unit and return its definitions as an anonymous immutable module value." },
    ...
    .{ .name = "@defm", .effect = "values body module-name --", .text = "Evaluate a module body with an explicit initial stack and register the resulting module value under a name; exactly `@module` followed by `register`." },
    ...
    .{ .name = "@each", .effect = "sequence values quotation -- results", .text = "Apply a quotation with an explicit shared initial stack concurrently in one fresh unit per element and return one result per element in input order." },
