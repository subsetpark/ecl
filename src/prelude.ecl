...
(cat dup len 0 = (pop ()) when) 'compose def

### def first
(list -- value : "Return the first element of a nonempty list.")
(dup len pop 0 at) 'first def
...
(wrap (first) cons) 'literal def

### def dip
(: "Run a quotation beneath a protected top stack value.")
(swap literal compose call) 'dip def
...
(swap literal swap compose) 'partial def
...
(values quotation -- quotation :
 "Return a quotation that pushes every value from a list, in order and inertly, before running
  ...
  This is ordinary quotation composition and nothing more. Unit constructors instead take their
  initial values and body as two separate operands, preserving the exact body for attribution.")
(((literal) each) dip append raze) 'with def

### def mod
(x y -- z : "Compute checked integer remainders pervasively.")
(over over div * -) 'mod def
...
(list seed quotation -- list :
 "Apply a binary quotation to each element and its predecessor, using the explicit seed before the
  first element.")
(|xs seed quotation| xs seed xs cons -1 drop quotation zip-with) 'each-prior def
...
(list quotation -- list :
 "Return a nonempty list's first element followed by successive accumulator values from reducing its
  remaining elements.")
(|xs quotation| xs first wrap xs rest xs first quotation scan cat) 'scan1 def
...
(|value count quotation|
 value wrap count range value (pop) quotation compose scan cat) 'iterations def
...
(|value predicate step|
 value predicate step (dup) compose unfold nip value swap cons) 'while-values def
