...
test "embedded prelude exposes source bodies and derived dataflow" {
    try support.expectStacks(&.{
        ...
        .{
            .name = "three-value stack shuffles",
            .source = "1 2 3 rollup 4 5 6 rolldown",
        ...
        .{
            ...
            .source = "[] (42) @attempt [2 3] (+) @attempt " ++
                "[2 3] (+) @spawn task.await [2 0] (/) @attempt result.ok? " ++
                "[2 3] (+) @attempt [2 3] (+) @spawn task.await match? " ++
        ...
        .{
            ...
            .source = "1 2 nip 3 (1 +) keep 3 (1 +) (2 *) bi 3 (1 +) (2 *) (1 -) tri " ++
                "2 3 (+) (*) bi2 2 3 (1 +) both 2 3 (+) (*) (-) tri2",
            .expected = "2 4 3 4 6 4 6 2 5 6 3 4 5 6 -1",
        },
        .{
        ...
        .{
            ...
            .source = "[1 2] [1 3] lex-cmp [1 3] [1 2] lex-cmp " ++
                "[1] [1 0] lex-cmp [1 0] [1] lex-cmp [] [] lex-cmp",
            .expected = "-1 1 -1 1 0",
        },
        .{
            .name = "custom lexicographic comparison stops at the first difference",
            .source = "0 'lex-calls set [1 2 3] [9 0 0] " ++
                "(lex-calls 1 + 'lex-calls set cmp) lex-cmp-with lex-calls",
            .expected = "-1 1",
        },
        .{
...
test "all embedded vocabulary entries expose nonempty documentation" {
    const names = [_][]const u8{
        ...
        "rollup",       "rolldown",  "partial",    "with",     "mod",          "neg",
        "abs",          "<>",        "<=",         ">=",       "and",          "or",
        "nip",          "keep",      "bi",         "tri",      "bi2",          "tri2",
        "both",         "when",      "unless",     "case",     "signum",       "clamp",
        "last",         "pair",      "pack",       "append",   "rest",         "reverse",
        "uncons",       "unappend",  "empty?",     "zip",      "lex-cmp-with", "lex-cmp",
        "min-of",       "max-of",    "sort",       "distinct", "at-path",      "at-or",
        "filter",       "partition", "any?",       "all?",     "sum",          "prod",
        "mean",         "fail",      "find",       "set",      "setp",         "assert",
