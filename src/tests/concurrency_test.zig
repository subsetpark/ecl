...
test "concurrency: task identity rendering dict keys and cached await are observable" {
    ...
    try runOk(
        ...
        "pop 'identity-task set " ++
            ...
            "identity-task 9 pair dict.from-flat identity-task dict.has?",
    );
    {
        var actual = try display(&runtime);
        defer actual.deinit();
...
test "concurrency: a failing sibling cancels peers mid-construction" {
    ...
    const failure = switch (try runtime.runUnit(
        "concurrency.ecl",
        "[0 1 2 3] [] ((0 =) (1 0 /) (30000 range (29999 (pop) times) @attempt pop 1) if) @each",
    )) {
        .err => |item| item,
...
test "concurrency: await-any ties and @each preserve program order" {
    ...
    actual.deinit();
    try runOk(&runtime, "pop [1 2 3] [] (dup *) @each");
    actual = try display(&runtime);
...
test "concurrency: primitive @each is reflective and task-join is absent" {
    ...
    try runOk(
        &runtime,
        "'@each doc " ++
            "\"Apply a quotation with an explicit shared initial stack concurrently in one fresh unit per element and return one result per element in input order.\" match? " ++
    ...
    try std.testing.expect(std.mem.indexOf(
        u8,
        output.buffered(),
        "@each -> @each primitive public (sequence values quotation -- results)\n",
    ) != null);
    try std.testing.expect(std.mem.indexOf(
        u8,
        output.buffered(),
